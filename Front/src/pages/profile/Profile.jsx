...
const Profile = () => {
 ...
 // Se crea esta función para confirmar la cancelación de una orden, donde se envía una petición PUT a la API para cambiar el estado de la orden a 'Canceled'
  const confirmCancel = async () => {
    try {
      const response = await fetch(`http://localhost:3000/api/orders/${orderToCancel.order_id}/cancel`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/orders/${selectedOrder.order_id}/update`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
